...
export default function RegulatorDashboard() {
  ...
  const fetchData = async () => {
    try {
      const [s, v, a, d, p] = await Promise.all([
        ...
        API.get("/alerts/"),
        API.get("/drugs/"),
        API.get("/drugs/?status=pending"),
      ]);
      setStats(s.data);
      setVerStats(v.data);
      setAlerts(a.data.results || a.data);
      setDrugs(d.data.results || d.data);
      setPendingDrugs(p.data.results || p.data);
  ...
  const approveDrug = async (id) => {
    try {
      await API.post("/drugs/" + id + "/approve/");
      fetchData();
      toast.success("Drug approved!");
    } catch(e) { toast.error("Failed to approve drug."); }
  };

  const rejectDrug = async (id) => {
    try {
      await API.post("/drugs/" + id + "/reject/");
      fetchData();
      toast.success("Drug rejected.");
    } catch(e) { toast.error("Failed to reject drug."); }
  };
  ...
  if (loading) return (
    <DashboardLayout title="Regulator Dashboard">
      <div className="flex items-center justify-center h-64">
        <div className="animate-spin rounded-full h-10 w-10 border-4 border-gray-200 border-t-purple-700"></div>
      </div>
    </DashboardLayout>
  );
    ...
    <DashboardLayout title="Regulator Dashboard" subtitle="Monitor all drugs, alerts and verifications across the system">
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {["overview","pending","drugs","alerts"].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={"px-4 py-2 rounded-xl text-sm font-semibold capitalize transition " + (activeTab === tab ? "bg-[#0A2647] text-white" : "bg-white dark:bg-gray-800 text-gray-500 border border-gray-200 hover:border-gray-300")}>
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><BarChart2 className="w-5 h-5" /> Verification Results</h2>
            {verStats.length > 0 ? (
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie data={verStats} dataKey="count" nameKey="result" cx="50%" cy="50%" outerRadius={90} label={({result, percent}) => result + " " + (percent*100).toFixed(0) + "%"}>
                    {verStats.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                  </Pie>
                  <Tooltip />
                </PieChart>
              </ResponsiveContainer>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><Bell className="w-5 h-5 text-red-500" /> Active Alerts</h2>
            <div className="space-y-3 max-h-56 overflow-y-auto">
              {alerts.filter(a => !a.is_resolved).length === 0 ? (
                ...
                <div key={alert.id} className="flex justify-between items-center p-4 bg-red-50 dark:bg-red-900 rounded-xl border border-red-100 dark:border-red-800">
                  <div>
      ...
      {activeTab === "pending" && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="p-6 border-b border-gray-100 dark:border-gray-700">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">Pending Drug Approvals</h2>
            <p className="text-sm text-gray-400 mt-0.5">{pendingDrugs.length} drug{pendingDrugs.length !== 1 ? "s" : ""} awaiting approval</p>
          </div>
          <div className="divide-y divide-gray-50 dark:divide-gray-700">
            {pendingDrugs.length === 0 ? (
              ...
              <div key={drug.id} className="p-6 flex justify-between items-center hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <p className="font-bold text-gray-900 dark:text-white text-lg">{drug.name}</p>
      ...
      {activeTab === "drugs" && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="p-6 border-b border-gray-100 dark:border-gray-700">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">All Drugs</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700 border-b border-gray-100 dark:border-gray-600">
                  {["Drug Name","NAFDAC No.","Manufacturer","Dosage Form","Status"].map(h => (
      ...
      {activeTab === "alerts" && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="p-6 border-b border-gray-100 dark:border-gray-700">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">All Alerts</h2>
          </div>
          <div className="p-6 space-y-3">
            {alerts.length === 0 ? (
              ...
              <div key={alert.id} className={"border rounded-xl p-4 flex justify-between items-start " + (alert.is_resolved ? "bg-gray-50 dark:bg-gray-700 border-gray-100 opacity-60" : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-600")}>
                <div>
