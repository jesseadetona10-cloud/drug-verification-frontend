        ...
        {showAdd && (
          <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} className="border-b border-gray-100 p-6 bg-blue-50 dark:bg-gray-700">
            <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1 uppercase tracking-wide">Drug</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1 uppercase tracking-wide">Alert Type</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1 uppercase tracking-wide">Severity</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1 uppercase tracking-wide">Title</label>
              ...
              <div className="md:col-span-2">
                <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1 uppercase tracking-wide">Message</label>
        ...
        <div className="p-6 space-y-3">
          {loading ? (
            <div className="flex justify-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-4 border-gray-200 border-t-blue-700"></div>
            </div>
          ) : alerts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-gray-300">
              <Bell className="w-16 h-16 mb-4" />
              <p className="text-gray-400 font-medium text-lg">No alerts yet</p>
              <p className="text-sm text-gray-400">Alerts will appear here when counterfeit drugs are detected</p>
            </div>
          ) : alerts.map(alert => (
            <motion.div key={alert.id} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}
              className={"border rounded-xl p-4 flex justify-between items-start " + (alert.is_resolved ? "bg-gray-50 dark:bg-gray-700 border-gray-100 opacity-60" : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-600")}>
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {alert.is_resolved && <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">Resolved</span>}
                  </div>
              ...
              {!alert.is_resolved && (
                <button onClick={() => resolveAlert(alert.id)} className="text-xs bg-emerald-100 text-emerald-700 border border-emerald-200 px-3 py-1.5 rounded-lg hover:bg-emerald-200 transition font-medium whitespace-nowrap ml-4">
                  Resolve
                </button>
              )}
            </motion.div>
