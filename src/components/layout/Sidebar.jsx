...
import { LayoutDashboard, Pill, Package, Bell, Search, ClipboardList, BarChart2, Users, LogOut, Shield } from "lucide-react";
    ...
    <div className={"w-64 min-h-screen bg-gradient-to-b " + gradient + " flex flex-col shadow-2xl"}>
      <div className="p-6 border-b border-white border-opacity-10">
        ...
        <div className="flex items-center gap-3 bg-white bg-opacity-10 rounded-xl p-3">
          <div className="w-10 h-10 rounded-full bg-white bg-opacity-20 flex items-center justify-center text-white font-bold text-lg">
            {user?.email?.[0]?.toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-white truncate">{user?.company_name || user?.email}</p>
            <p className="text-xs text-white text-opacity-50 capitalize">{user?.role}</p>
          </div>
        </div>
      </div>
      <nav className="flex-1 p-4 space-y-1">
        <p className="text-xs text-white text-opacity-30 uppercase tracking-widest px-3 mb-3 font-semibold">Navigation</p>
        {items.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
            ...
            className={({isActive}) =>
              ...
              (isActive ? "bg-white bg-opacity-20 text-white font-semibold shadow" : "text-white text-opacity-50 hover:bg-white hover:bg-opacity-10 hover:text-white")
            }
          >
      ...
      <div className="p-4 border-t border-white border-opacity-10">
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 text-white text-opacity-50 hover:text-white hover:bg-white hover:bg-opacity-10 rounded-xl transition"
        >
